...
export function isCastSdkEnabled(): boolean {
  return import.meta.env.VITE_CAST_SDK_ENABLED === 'true';
}
...
export function getCastSessionController(): CastSessionController {
  ...
  if (!appId) {
    console.warn('[Cast] VITE_CAST_SDK_ENABLED is true but VITE_CAST_APP_ID is missing. Falling back to no-op.');
    singleton = new NoopCastSessionController();
    return singleton;
  }
